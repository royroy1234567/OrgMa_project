...
<div class="flex">
    ...
    <main class="flex-1 mt-16 p-6 md:ml-64 transition-all duration-300">
        ...
        <!-- ── PAGE HEADING ──────────────────────────────── -->
        <div class="mb-6">
            <h2 class="text-lg font-semibold text-gray-800">Reorder Recommendation</h2>
        </div>
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-50">
                        <tr class="border-b border-gray-200">
                            ...
                            <th class="text-left py-3 px-4 text-xs text-gray-500 font-semibold uppercase tracking-wider">Current Stock</th>
                            <th class="text-left py-3 px-4 text-xs text-gray-500 font-semibold uppercase tracking-wider">Days Left</th>
                            <th class="text-left py-3 px-4 text-xs text-gray-500 font-semibold uppercase tracking-wider">Recommended Qty</th>
                            <th class="text-left py-3 px-4 text-xs text-gray-500 font-semibold uppercase tracking-wider">Supplier</th>
                            <th class="text-left py-3 px-4 text-xs text-gray-500 font-semibold uppercase tracking-wider">Est. Cost</th>
                    ...
                    <tbody>
                        @php
                            $reorders = [
                                ['product' => 'TCL TV',                    'current' => 15, 'days' => 4, 'qty' => 10, 'supplier' => 'TCL',       'cost' => '₱ 40,000'],
                                ['product' => 'LG Refrigerator',           'current' => 8,  'days' => 2, 'qty' => 7,  'supplier' => 'LG',        'cost' => '₱ 90,000'],
                                ['product' => 'Astron Rice Cooker',        'current' => 6,  'days' => 3, 'qty' => 4,  'supplier' => 'Astron',    'cost' => '₱ 10,000'],
                                ['product' => 'Panasonic Washing Machine', 'current' => 4,  'days' => 4, 'qty' => 6,  'supplier' => 'Panasonic', 'cost' => '₱ 60,000'],
                            ];
                        @endphp

                        @foreach ($reorders as $r)
                        <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                            <td class="py-3.5 px-4 font-medium text-gray-800">{{ $r['product'] }}</td>
                            <td class="py-3.5 px-4 text-gray-700">{{ $r['current'] }}</td>
                            <td class="py-3.5 px-4 text-gray-700">{{ $r['days'] }}</td>
                            <td class="py-3.5 px-4 text-gray-700">{{ $r['qty'] }}</td>
                            <td class="py-3.5 px-4 text-gray-700">{{ $r['supplier'] }}</td>
                            <td class="py-3.5 px-4 font-semibold text-gray-800">{{ $r['cost'] }}</td>
